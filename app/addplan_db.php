    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['budget']) && isset($_POST['start']) && isset($_POST['end'])) {
            ...
            try {
                ...
                // 値をバインドしてSQLを実行
                $stmt->bindValue(':event_id', $event_id1, PDO::PARAM_INT); // 更新するイベントのID
                $stmt->bindValue(':route_id', $route_id, PDO::PARAM_INT); // 更新するルートのID
                $stmt->bindValue(':travel_id', $travel_id, PDO::PARAM_INT); // 外部キーとして旅行ID
                $stmt->bindValue(':charge', $charge, PDO::PARAM_INT); // 料金
                $stmt->bindValue(':start_datetime', $start_datetime, PDO::PARAM_STR); // 開始日時
                $stmt->bindValue(':end_datetime', $end_datetime, PDO::PARAM_STR); // 終了日時
                $stmt->bindValue(':transport_id', $vehicle, PDO::PARAM_INT); // 交通手段のID
                $stmt->bindValue(':place', $place, PDO::PARAM_STR); // 開催場所
                $stmt->bindValue(':event_detail', $event_detail, PDO::PARAM_STR); // イベント詳細
                $stmt->execute();
            ...
            // 値をバインドしてSQLを実行
            $stmt->bindValue(':event_id', $event_id2, PDO::PARAM_INT); // 更新するイベントのID
            $stmt->bindValue(':route_id', $route_id, PDO::PARAM_INT); // 更新するルートのID
            $stmt->bindValue(':travel_id', $travel_id, PDO::PARAM_INT); // 外部キーとして旅行ID
            $stmt->bindValue(':charge', $charge, PDO::PARAM_INT); // 料金
            $stmt->bindValue(':start_datetime', $start_datetime, PDO::PARAM_STR); // 開始日時
